...
//NAMADD GALVO mcp4728
#if ENABLED (GALVO_LASER_I2C)
	#include "mcp4728.h"
	mcp4728 dac_mcp4728 = mcp4728(0); // instantiate mcp4728 object, Device ID = 0
#endif
...
//NAMADD Z TRIAD
  ...
  static bool performing_homing = false,
              locked_z_motor = false,
              locked_z2_motor = false,
			  locked_z3_motor = false;
  bool bed_level_point_a=false,bed_level_point_b=false,bed_level_point_c=false;
#endif
...
#if (DISABLED(Z_DUAL_ENDSTOPS)||DISABLED(Z_TRIAD_ENDSTOPS))
  static byte
#else
  static uint16_t
#endif
...
//===========================================================================
...
#elif ENABLED(Z_TRIAD_STEPPER_DRIVERS) //NAMADD Z TRIAD
  #define Z_APPLY_DIR(v,Q) { Z_DIR_WRITE(v); Z2_DIR_WRITE(v); Z3_DIR_WRITE(v); }
    ...
    if (performing_homing) { \
      if (Z_HOME_DIR > 0) {\
		...
		if (bed_level_point_a)\
			if (!(TEST(old_endstop_bits, Z_MIN) && (count_direction[Z_AXIS] < 0)) && !locked_z_motor) Z_STEP_WRITE(v); \
		if (bed_level_point_b)\
			if (!(TEST(old_endstop_bits, Z2_MIN) && (count_direction[Z_AXIS] < 0)) && !locked_z2_motor) Z2_STEP_WRITE(v); \
		if (bed_level_point_c)\
			if (!(TEST(old_endstop_bits, Z3_MIN) && (count_direction[Z_AXIS] < 0)) && !locked_z3_motor) Z3_STEP_WRITE(v); \
		if (!bed_level_point_a && !bed_level_point_b && !bed_level_point_c) { \
			if (!(TEST(old_endstop_bits, Z_MIN) && (count_direction[Z_AXIS] < 0)) && !locked_z_motor) Z_STEP_WRITE(v); \
			if (!(TEST(old_endstop_bits, Z2_MIN) && (count_direction[Z_AXIS] < 0)) && !locked_z2_motor) Z2_STEP_WRITE(v); \
			if (!(TEST(old_endstop_bits, Z3_MIN) && (count_direction[Z_AXIS] < 0)) && !locked_z3_motor) Z3_STEP_WRITE(v); \
		} \
  ...
  #else
    #define Z_APPLY_STEP(v,Q) { Z_STEP_WRITE(v); Z2_STEP_WRITE(v); Z3_STEP_WRITE(v); }
  #endif
...
inline void update_endstops() {
  ...
  #if (ENABLED(Z_DUAL_ENDSTOPS)||ENABLED(Z_TRIAD_ENDSTOPS))
    uint16_t
  #else
    byte
  #endif
  ...
  #define UPDATE_ENDSTOP(AXIS,MINMAX) do { \
    ...
    if ((current_block->steps[A_AXIS] != current_block->steps[C_AXIS]) || (TEST(out_bits, A_AXIS) != TEST(out_bits, C_AXIS))) {
      ...
      { // z -direction
		  ...
		  #elif ENABLED(Z_TRIAD_ENDSTOPS) //NAMADD Z TRIAD
		  		  
		    SET_ENDSTOP_BIT(Z, MIN);
            #if (HAS_Z2_MIN && HAS_Z3_MIN)
			  SET_ENDSTOP_BIT(Z2, MIN);
			  SET_ENDSTOP_BIT(Z3, MIN);		  
			  SET_ENDSTOP_BIT(Z4, MIN);
            #else
			  COPY_BIT(current_endstop_bits, Z_MIN, Z2_MIN);
            ...
            byte z_test = TEST_ENDSTOP(Z_MIN) | (TEST_ENDSTOP(Z2_MIN) << 1) | (TEST_ENDSTOP(Z3_MIN) << 2) | (TEST_ENDSTOP(Z4_MIN) << 3);  // bit 0 for Z, bit 1 for Z2, bit 2 for z3

            if (z_test && current_block->steps[Z_AXIS] > 0) { // z_test = Z_MIN || Z2_MIN
              endstops_trigsteps[Z_AXIS] = count_position[Z_AXIS];
              SBI(endstop_hit_bits, Z_MIN);
              if (!performing_homing || (z_test >= 0x7))  //NAMFIX Z TRIAD //if not performing home or if both endstops were trigged during homing...
			  {	  
				  step_events_completed = current_block->step_event_count; 
		  ...
		  #elif ENABLED(Z_TRIAD_ENDSTOPS) //NAMADD Z TRIAD
            SET_ENDSTOP_BIT(Z, MAX);
            #if (HAS_Z2_MAX && HAS_Z3_MAX)
              SET_ENDSTOP_BIT(Z2, MAX);
			  SET_ENDSTOP_BIT(Z3, MAX);
            #else
              COPY_BIT(current_endstop_bits, Z_MAX, Z2_MAX);
			  COPY_BIT(current_endstop_bits, Z_MAX, Z3_MAX);
            #endif

            byte z_test = TEST_ENDSTOP(Z_MAX) | (TEST_ENDSTOP(Z2_MAX) << 1) | (TEST_ENDSTOP(Z3_MAX) << 2); // bit 0 for Z, bit 1 for Z2
          ...
          #else // !Z_DUAL_ENDSTOPS && !Z_TRIAD_ENDSTOPS
...
ISR(TIMER1_COMPA_vect) {
  ...
  if (current_block != NULL) {
    ...
    for (int8_t i = 0; i < step_loops; i++) {
      ...
      STEP_ADD(z,Z);
      #if DISABLED(ADVANCE)
		#if DISABLED(GALVO_LASER) //NAMADD
			STEP_ADD(e,E);  
		#endif
      #endif
	  ...
	  #if ENABLED(GALVO_LASER)		  //NAMADD GALVO	
		#define STEP_IF_COUNTER_X_GALVO(axis, AXIS) \
		if (_COUNTER(axis) > 0) { \
			_COUNTER(axis) -= current_block->step_event_count; \
			update_X_galvo(count_direction[_AXIS(AXIS)]); \
		}\
		
		#define STEP_IF_COUNTER_Y_GALVO(axis, AXIS) \
		if (_COUNTER(axis) > 0) { \
		_COUNTER(axis) -= current_block->step_event_count; \
		update_Y_galvo(count_direction[_AXIS(AXIS)]); \
		}\			
		STEP_IF_COUNTER_X_GALVO(x, X);  				
		STEP_IF_COUNTER_Y_GALVO(y, Y);
...
/*  	//Scanning X&Y With Galvos!      
      ...
      unsigned long old_y = Galvo_WorldYPosition;
       ...
       if (counter_x > 0) { 
			...
			//Galvo_WorldXPosition += (count_direction[X_AXIS]);
			update_X_galvo(count_direction[X_AXIS]);			
			//X_APPLY_STEP(INVERT_X_DIR,0);
	  }
	  
      if (counter_y > 0) { 
			counter_y-= current_block->step_event_count;       
			//Galvo_WorldYPosition += (count_direction[Y_AXIS]);
			update_Y_galvo(count_direction[Y_AXIS]);	
			//Y_APPLY_STEP(INVERT_Y_DIR,0);
	  } 
  
      //scan_XY_galvo(old_x, old_y, Galvo_WorldXPosition, Galvo_WorldYPosition);
...
void st_init() {
  ...
  // Initialize Dir Pins
    ...
    #endif
	//NAMADD Z TRIAD
    #if ENABLED(Z_TRIAD_STEPPER_DRIVERS) && HAS_Z2_DIR && HAS_Z3_DIR
	  Z2_DIR_INIT;
      Z3_DIR_INIT;
    #endif	
  ...
  //Initialize Enable Pins - steppers default to disabled.
	...
	#if ENABLED(Z_TRIAD_STEPPER_DRIVERS) && HAS_Z2_ENABLE && HAS_Z3_ENABLE
      Z2_ENABLE_INIT;
	  Z3_ENABLE_INIT;
      if (!Z_ENABLE_ON) {Z2_ENABLE_WRITE(HIGH);Z3_ENABLE_WRITE(HIGH);}
    #endif
...
//NAMADD Z TRIAD
  ...
  // Initialize Step Pins
    ...
    #endif
	//NAMADD Z TRIAD
    #if ENABLED(Z_TRIAD_STEPPER_DRIVERS) && HAS_Z2_STEP && HAS_Z3_STEP
      Z2_STEP_INIT;
      Z2_STEP_WRITE(INVERT_Z_STEP_PIN);
 ...
 */
void st_synchronize() { while (blocks_queued()) {idle(); lifetime_stats_tick();} }

// XY Galvo setup NAMFIX
#if ENABLED(GALVO_LASER)
	#if ENABLED (GALVO_LASER_SPI)
		static void galvo_sw_spi_send(uint8_t val) {
		  uint8_t i;
		  for (i = 0; i < 8; i++) {
			WRITE(GALVO_LR_CLK_PIN,0);
			#if F_CPU == 20000000
			  __asm__("nop\n\t");
			#endif
			WRITE(GALVO_DAT_PIN,val&0x80);
			val<<=1;
			WRITE(GALVO_LR_CLK_PIN,1);
			#if F_CPU == 20000000
			  __asm__("nop\n\t""nop\n\t");
			#endif
		  }
		...
		void galvos_digitalPotWrite(int channel, int value) {
		  ...
		  digitalWrite(GALVO_CS_PIN,LOW);
		  //  send in the address and value via SPI:
		  //SPI.transfer(channel);
		  //SPI.transfer(value);
		  galvo_sw_spi_send(channel);
		  galvo_sw_spi_send(value);
		  // take the SS pin high to de-select the chip:
		  digitalWrite(GALVO_CS_PIN,HIGH);
		}
		
		void move_X_galvo(unsigned short X)
		{
		  unsigned char xHigh = (((X) & 0x0F00) >> 8);
		  unsigned char xLow  = (X) & 0x00FF;
		  xHigh = xHigh | 0x30;     // 0b0011 0000 : kênh 0/buffer/gain(1=Voutx1,0=Voutx2)/SHDN luôn =1 , 4 bit sau là bit dữ liệu
		  galvos_digitalPotWrite(xHigh, xLow);
		}

		void move_Y_galvo(unsigned short Y)
		{
		  unsigned char yHigh = ((Y)  & 0x0F00) >> 8;
		  unsigned char yLow  = (Y)  & 0x00FF;
		  yHigh = yHigh | 0xB0;     // 0b1011 0000 : kênh 1/buffer/gain(1=Voutx1,0=Voutx2)/SHDN luôn =1 , 4 bit sau là bit dữ liệu
		  galvos_digitalPotWrite(yHigh, yLow);
		}			
	#elif ENABLED (GALVO_LASER_I2C)
		void galvos_digitalPotWrite(int channel, int value) {
		  dac_mcp4728.analogWrite(channel,value);
		}
		
		void move_X_galvo(unsigned short X)
		{
		 galvos_digitalPotWrite(0, X);
		}

		void move_Y_galvo(unsigned short Y)
		{
		  galvos_digitalPotWrite(1, Y);
		}		
	#endif	
	...
	void update_X_galvo(int step_dir)
	{
	   ...
	   if(Galvo_WorldXPosition > 0x00000FFF)
	   {
		  s = 0x00000FFF;
	   }
	   move_X_galvo(s);
	...
	void update_Y_galvo(int step_dir)
	{
	   ...
	   if(Galvo_WorldYPosition > 0x00000FFF)
	   {
		  s = 0x00000FFF;
	   }
...
//NAMADD Z TRIAD
  ...
  void Lock_z_motor(bool state) { locked_z_motor = state; }
  void Lock_z2_motor(bool state) { locked_z2_motor = state; }
  void Lock_z3_motor(bool state) { locked_z3_motor = state; }
#endif
