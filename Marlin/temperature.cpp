...
unsigned char soft_pwm_bed;
  ...
  void thermal_runaway_maxterm_protection(TRState* state, millis_t* timer, float temperature, float target_temperature, int heater_id, int period_seconds, int hysteresis_degc);
...
void manage_heater() {
  ...
  for (int e = 0; e < EXTRUDERS; e++) {
    ...
    #if ENABLED(THERMAL_PROTECTION_HOTENDS)
      thermal_runaway_maxterm_protection(&thermal_runaway_state_machine[e], &thermal_runaway_timer[e], current_temperature[e], target_temperature[e], e, THERMAL_PROTECTION_PERIOD, THERMAL_PROTECTION_HYSTERESIS);
    #endif
  ...
  // Control the extruder rate based on the width sensor
    ...
    next_bed_check_ms = ms + BED_CHECK_INTERVAL;
      ...
      thermal_runaway_maxterm_protection(&thermal_runaway_bed_state_machine, &thermal_runaway_bed_timer, current_temperature_bed, target_temperature_bed, -1, THERMAL_PROTECTION_BED_PERIOD, THERMAL_PROTECTION_BED_HYSTERESIS);
    #endif
