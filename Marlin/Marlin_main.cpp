...
#if ENABLED(GALVO_LASER_SPI) //NAMADD
  #include <SPI.h>
#endif


#if ENABLED(GALVO_LASER_I2C) //NAMADD
  #include <Wire.h>
#endif
 ...
 * M999 - Restart after being stopped by error
	///NAMADD
 * M002 - Laser control-on  //NAMADD
 * M003 - Laser control-off  //NAMADD
 * M130 - Layer change  //NAMADD
 * M131 - Roll //NAMADD
 * M132 - Slice //NAMADD
 * M010 - On/Off  Filament check, P[Extruder (1-3)] S[True/False = 1/0]
   M009 - Led ring, P[Color(1,2,3,4,5,6,7=RGBYPCW)] S[state]
   M005 - Complete state
   M006 - Pause
   M007 - Pause for max temp protect machine
   M008 - Wait for over heat protection
   ______
   M011 - FDM_Z_TRIAD HOME ALL
   M012 - FDM_Z_TRIAD LEVEL BED
 *
 * "T" Codes
 *
...
//NAMADD FDM200_DIAMOND
static float destination_temp[NUM_AXIS] = { 0.0 };
...
unsigned long filament_check_delay;  
...
//NAMADD max temp protect
	...
	bool maxtemp_protect_en=true;
	bool maxtemp_protect_status=false;
	uint16_t NOZZLE_MAXTEMP_PROTECT=HEATER_0_MAXTEMP-30;
	#if HAS_TEMP_BED
		uint16_t BED_MAXTEMP_PROTECT=BED_MAXTEMP-5;
	#endif
	uint16_t X_MAXTEMP_PROTECT=0;
	uint16_t Y_MAXTEMP_PROTECT=0;
	uint16_t Z_MAXTEMP_PROTECT=1;	
#endif

//NAMADD
#if ENABLED(SDSUPPORT)
	char name_file_is_printing[27];
#endif
...
//NAMADD
	...
	int LASER_POWER = 0;
	int laserPower = 0;
	bool Laser_Always_On = true;
	#if ENABLED (SLS_PRINTER)
		bool sls_dir=false;   
		int sls_steps; 
		unsigned int sls_delay;
	#endif
#endif
...
//NAMADD Z TRIAD
#if (ENABLED(Z_TRIAD_ENDSTOPS)&&DISABLED(DELTA))
  float z_endstop_adj = 0;
  extern bool bed_level_point_a;
  extern bool bed_level_point_b;
  extern bool bed_level_point_c;
#endif
...
void enqueue_and_echo_command_now(const char* cmd) {
  while (!enqueue_and_echo_command(cmd)) {idle();lifetime_stats_tick();}
}
...
//NAMADD LED_RING_LOW_V1
	...
	void setup_led_ring_low_v1()
	{
		SET_OUTPUT(LR_RST_PIN);
		SET_OUTPUT(LR_CLK_PIN);
		SET_OUTPUT(LR_LATCH_PIN);
		SET_OUTPUT(LR_RED_PIN);
		SET_OUTPUT(LR_GREEN_PIN);
		SET_OUTPUT(LR_BLUE_PIN);

		digitalWrite(LR_RST_PIN,HIGH);
		
		digitalWrite(LR_RED_PIN,HIGH);
		digitalWrite(LR_GREEN_PIN,HIGH);
		digitalWrite(LR_BLUE_PIN,HIGH);
...
//NAMADD LED_RING_HIGH_V2
	...
	void setup_led_ring_high_v2()
	{
		...
		digitalWrite(LR_RST_PIN,LOW); //Reset led ring
		
		digitalWrite(LR_RED_PIN,LOW);
		digitalWrite(LR_GREEN_PIN,LOW);
		digitalWrite(LR_BLUE_PIN,LOW);
		
		digitalWrite(LR_RST_PIN,HIGH); //UnReset led ring			
	}
#endif

//NAMADD
#if ENABLED (GALVO_LASER)
	void setup_galvos()	 //NAMADD GALVO setup_galvos
	{
		#if ENABLED (GALVO_LASER_SPI)
			#if (GALVO_CS_PIN > -1)
				SET_OUTPUT(GALVO_CS_PIN);
				SET_OUTPUT(GALVO_LR_CLK_PIN);
				SET_OUTPUT(GALVO_DAT_PIN);
				//SPI.begin();   
				//SPI.setBitOrder(MSBFIRST);	  
				//Timer1.initialize(1000); //1000/16Mhz
				//Timer1.attachInterrupt(timed_refresh_of_galvos); // blinkLED to run every 0.15 seconds
			#endif		
		#elif ENABLED(GALVO_LASER_I2C)
			Wire.begin();
		#endif
	  ...
	  #if ENABLED(SLS_PRINTER)
		  #if (S_STEP_PIN>-1)
			SET_OUTPUT(S_STEP_PIN);
			SET_OUTPUT(S_DIR_PIN);
			SET_OUTPUT(S_ENABLE_PIN);
					
			SET_INPUT(S_ENDSTOP_MIN_PIN);
			SET_INPUT(S_ENDSTOP_MAX_PIN);
		  #endif
		...
		digitalWrite(S_ENABLE_PIN,HIGH);
...
#if ENABLED(LASER_ENGRAVING_MODULE)
void setup_laser_engraving_module()	 //NAMADD
{
	#if (LASER_PIN > -1) 
		SET_OUTPUT(LASER_PIN);
	#endif 
...
//NAMADD LED_RING_LOW_V1
  #if ENABLED(LED_RING_LOW_V1)
	setup_led_ring_low_v1();
  #elif ENABLED(LED_RING_HIGH_V2)
	setup_led_ring_high_v2();
  #endif
  ...
  //NAMADD LED_RING
	...
	//Option 1
	/*
	led_ring_low_v1_init_1();
	led_ring_low_v1_blinking(1,0,2);
	led_ring_low_v1_blinking(4,0,2);
	led_ring_low_v1_blinking(2,0,2);
	*/
	
	//Option 2
	led_ring_low_v1_init_2(0);
	led_ring_low_v1_blinking(1,0,2);
	led_ring_low_v1_blinking(4,0,2);
	led_ring_low_v1_blinking(2,0,2);
  #elif ENABLED(LED_RING_HIGH_V2)	
	led_ring_high_v2_shift_one_led(1,15);
	led_ring_high_v2_shift_one_led(3,15);	
	led_ring_high_v2_shift_one_led(2,15);
  #endif 
}
...
void loop() {
	...
	//NAMADD LED_RING
	#if ENABLED(LED_RING)
		if(led_ring_blink_en)
			#if ENABLED(LED_RING_LOW_V1)
				led_ring_low_v1_blinking(led_ring_color,led_ring_time,led_ring_number);
			#elif ENABLED(LED_RING_HIGH_V2)		
				led_ring_high_v2_blinking(led_ring_color,led_ring_time,led_ring_number);		
			#endif 
	#endif		
		//led_ring_high_v2_sdcard_printing(4,8);  
  ...
  if (axis == X_AXIS ? HOMEAXIS_DO(X) : axis == Y_AXIS ? HOMEAXIS_DO(Y) : axis == Z_AXIS ? HOMEAXIS_DO(Z) : 0) {
	...
	//NAMADD Z TRIAD
      ...
      if (axis == Z_AXIS) {
        ...
        if (lockZ1) Lock_z_motor(true); else {Lock_z2_motor(true);Lock_z3_motor(true);}
        sync_plan_position();
        ...
        if (lockZ1) Lock_z_motor(false); else {Lock_z2_motor(false);Lock_z3_motor(false);}
        In_Homing_Process(false);
      } // Z_AXIS
...
//NAMADD :Khoa hướng động cơ cho máy nhỏ
//#define FDM200_DIAMOND
// #if ENABLED(FDM200_DIAMOND)	
//  for (int i = 0; i < NUM_AXIS; i++) {
//E
    // if (code_seen(axis_codes[3]))
      // destination[3] = code_value() + (axis_relative_modes[3] || relative_mode ? current_position[3] : 0);		
    // else
      // destination[3] = current_position[3]; 
//Z
    // if (code_seen(axis_codes[2]))		
		// destination[2] = code_value() + (axis_relative_modes[2] || relative_mode ? current_position[2] : 0);		
    // else
...
//Y  
    // if (code_seen(axis_codes[1]))
		...
		// else
			// destination[1] = code_value() + (axis_relative_modes[1] || relative_mode ? current_position[1] : 0);		
    // else
      // destination[1] = current_position[1]; 
//X 
    // if (code_seen(axis_codes[0]))
		// if (destination[1]<70 || destination[2]<15)
			// destination[0]=current_position[0];
		// else
			// destination[0] = code_value() + (axis_relative_modes[0] || relative_mode ? current_position[0] : 0);		
    // else
      // destination[0] = current_position[0]; 
//  }	
// #else
    ...
    if (host_keepalive_interval && busy_state != NOT_BUSY) {
      ...
      switch (busy_state) {
        ...
        case PAUSED_FOR_USER:
          ...
          break;
        case PAUSED_FOR_OVERHEAT:
          SERIAL_ECHO_START;
          SERIAL_ECHOLNPGM(MSG_BUSY_PAUSED_FOR_OVERHEAT);
          break;		  		  
...
inline void gcode_G0_G1() {
  if (IsRunning()) {
	#if (ENABLED(GALVO_LASER))  //NAMADD
		//laserPower=(code_seen('E') && (code_value() > 0))?laserPower:0;
		laserPower=LASER_POWER;	
	#endif
	#if (ENABLED(LASER_ENGRAVING_MODULE))  //NAMADD
		laserPower=(code_seen('E') && (code_value() > 0))?LASER_POWER:0;
		//laserPower=LASER_POWER;	
	#endif
...
/**
 *NAMADD
 * G0: Coordinated movement of X Y Z E axes
 */
inline void gcode_G0() {
  if (IsRunning()) {
	#if (ENABLED(GALVO_LASER)||ENABLED(LASER_ENGRAVING_MODULE))  //NAMADD
		laserPower=0;	
	#endif
    gcode_get_destination(); // For X Y Z E F
...
/**
 *NAMADD
 * G1: Coordinated movement of X Y Z E axes
 */
inline void gcode_G1() {
  if (IsRunning()) {
	#if (ENABLED(GALVO_LASER))  //NAMADD
		laserPower=LASER_POWER;	
	#endif
...
inline void gcode_G4() {
  ...
  while (PENDING(millis(), codenum)) {idle();lifetime_stats_tick();}
}
...
inline void gcode_G92() {
  ...
  inline void gcode_M0_M1() {
    ...
    if (codenum > 0) {
      ...
      while (PENDING(millis(), codenum) && !lcd_clicked()) {idle();lifetime_stats_tick();}
      KEEPALIVE_STATE(IN_HANDLER);
...
inline void gcode_M17() {
  ...
  inline void gcode_M25() {
	  //NAMFIX
	  card.pauseSDPrint();	
	  enqueue_and_echo_commands_P(PSTR("G0 F2000 X0 Y0"));	  	
  }
...
inline void gcode_M105() {
  ...
  inline void gcode_M106() {
	...
	fanValue=p==1?fanSpeeds[p]:fanValue;
  }
...
inline void gcode_M109() {
  ...
  do {
	...
	//NAMADD LED_RING
	#if ENABLED(LED_RING)
		if(led_ring_heated_en)
			#if ENABLED(LED_RING_LOW_V1)
				led_ring_blink_en=false;
				led_ring_low_v1_sdcard_printing(led_ring_heated_color,(((unsigned int)degHotend(target_extruder)*100)/((unsigned int)degTargetHotend(target_extruder)*6)+1));
			#elif ENABLED(LED_RING_HIGH_V2)
				led_ring_blink_en=false;
				led_ring_high_v2_sdcard_printing(led_ring_heated_color,(((unsigned int)degHotend(target_extruder)*100)/((unsigned int)degTargetHotend(target_extruder)*6)));	
			#endif
	#endif
  ...
  inline void gcode_M190() {
	...
	//NAMADD LED_RING
		...
		if(led_ring_heated_en)
			#if ENABLED(LED_RING_LOW_V1)
				led_ring_blink_en=false;
				led_ring_low_v1_sdcard_printing(led_ring_heated_color,(((unsigned int)degBed()*100)/((unsigned int)degTargetBed()*6)+1));
			#elif ENABLED(LED_RING_HIGH_V2)
				led_ring_blink_en=false;
				led_ring_high_v2_sdcard_printing(led_ring_heated_color,(((unsigned int)degBed()*100)/((unsigned int)degTargetBed()*6)));	
			#endif
	#endif	
      now = millis();
...
inline void gcode_M119() {
  ...
  #endif 
  #if HAS_Z4_MIN
    SERIAL_PROTOCOLPGM(MSG_Z4_MIN);
    SERIAL_PROTOCOLLN(((READ(Z4_MIN_PIN)^Z4_MIN_ENDSTOP_INVERTING) ? MSG_ENDSTOP_HIT : MSG_ENDSTOP_OPEN));
  #endif    
...
#elif ENABLED(Z_TRIAD_ENDSTOPS) // !DELTA && ENABLED(Z_TRIAD_ENDSTOPS) && DISABLED(Z_DUAL_ENDSTOPS)
...
#endif // // !DELTA && ENABLED(Z_TRIAD_ENDSTOPS) && DISABLED(Z_DUAL_ENDSTOPS)
...
inline void gcode_M226() {
  if (code_seen('P')) {
    ...
    if (pin_state >= -1 && pin_state <= 1) {
      ...
      if (pin_number > -1) {
        ...
        while (digitalRead(pin_number) != target) {idle();lifetime_stats_tick();}
  ...
  inline void gcode_M300() {
    ...
    if (beepP > 10000) beepP = 10000; // limit to 5 seconds
    buzz(beepP, beepS);
  }
...
inline void gcode_M503() {
  ...
  inline void gcode_M600() {
    ...
    lcd_reset_alert_level();
      ...
      plan_buffer_line(delta[X_AXIS], delta[Y_AXIS], delta[Z_AXIS], destination[E_AXIS], feedrate / 60, active_extruder);
      plan_buffer_line(delta[X_AXIS], delta[Y_AXIS], delta[Z_AXIS], lastpos[E_AXIS], feedrate / 60, active_extruder);
    #else
...
//NAMADD LED_RING
#if ENABLED(LED_RING)
  inline void gcode_M009() {  
    if (code_seen('P'))
	{	
		led_ring_color=code_seen('P')?code_value_short():0;
		led_ring_time=code_seen('S')?code_value_short():0;
		led_ring_number=code_seen('N')?code_value_short():0;
	...
	else if (code_seen('D'))
	{
		led_ring_blink_en=false;
		#if ENABLED(LED_RING_LOW_V1)
			led_ring_low_v1_init_2(code_seen('D')?code_value_short():0);
		#elif ENABLED(LED_RING_HIGH_V2)
			led_ring_high_v2_shift_one_led(code_seen('D')?code_value_short():1,code_seen('T')?code_value_short()<=100?code_value_short():100:20);
		#endif
	}
	else if (code_seen('H'))
	{		
		led_ring_blink_en=false;
			...
			led_ring_low_v1_init_1();	
			//option 2
			//led_ring_low_v1_init_2(code_seen('T')?code_value_short():0);
		#elif ENABLED(LED_RING_HIGH_V2)
			led_ring_high_v2_shift_two_led_opposite(code_seen('H')?code_value_short():1,code_seen('T')?code_value_short()<=100?code_value_short():100:20);
		#endif
	}
	else if (code_seen('T'))
	{
		led_ring_heated_color=code_seen('T')?code_value_short():0;
		led_ring_blink_en=false;
		led_ring_heated_en=true;
	}	
	else 
	{
		led_ring_blink_en=false;
		led_ring_heated_en=false;
	}
  }
#endif // LED_RING
 ...
 */
//NAMADD Filament check
inline void	gcode_M010(){
    if (code_seen('P')) 		
		if (code_value_short()==1) {		
			filament_check_enabled_a = code_seen('S')?code_value_short():filament_check_enabled_a;
		}		
		else if (code_value_short()==2) {	
			filament_check_enabled_b = code_seen('S')?code_value_short():filament_check_enabled_b;
		}
		else if (code_value_short()==3) {	
			filament_check_enabled_c = code_seen('S')?code_value_short():filament_check_enabled_c;
		}
		else{
			filament_check_enabled_a=false;
			filament_check_enabled_b=false;
			filament_check_enabled_c=false;	  
		}
	else{
...
//NAMADD Complete state
	inline void	gcode_M005(){	
		float complete_value_normal = current_position[Z_AXIS]+1;
		if (code_seen('W'))  //Used for code
		{			
			//move Z
			destination[Z_AXIS] = complete_value;
			RUNPLAN;
		}
		else
		{
			complete_state=true;
			complete_value=code_seen('Z')?code_value():complete_value_normal;
		}
	
	}
	
//NAMADD Pause
	inline void	gcode_M006(){
		if(IS_SD_PRINTING){			
			card.pauseSDPrint();	
			//enqueue_and_echo_commands_P(PSTR("G0 F2000 X0 Y0"));			
...
//NAMADD MAXTEMP_PROTECT
#if ENABLED(MAXTEMP_PROTECT)
	inline void gcode_M007(){
		NOZZLE_MAXTEMP_PROTECT=code_seen('N')?constrain(code_value_short(),100,HEATER_0_MAXTEMP-30):NOZZLE_MAXTEMP_PROTECT;
		#if HAS_TEMP_BED
			BED_MAXTEMP_PROTECT=code_seen('B')?constrain(code_value_short(),40,BED_MAXTEMP-5):BED_MAXTEMP_PROTECT;
		#endif
		X_MAXTEMP_PROTECT=code_seen('X')?code_value_short():X_MAXTEMP_PROTECT;
		Y_MAXTEMP_PROTECT=code_seen('Y')?code_value_short():Y_MAXTEMP_PROTECT;
		Z_MAXTEMP_PROTECT=code_seen('Z')?code_value_short():Z_MAXTEMP_PROTECT;			
	}
	...
	//NAMADD
	   ...
	   * M1: // M1 - Conditional stop - Wait for user button press on LCD
	   */
	  inline void gcode_M008() { //sem sem M0_M1

		//NAMADD	  
		//lift Z
		if (code_seen('Z')) destination[Z_AXIS] += Z_MAXTEMP_PROTECT;
		RUNPLAN;
		//move xy
		if (code_seen('X')) destination[X_AXIS] = X_MAXTEMP_PROTECT;
		if (code_seen('Y')) destination[Y_AXIS] = Y_MAXTEMP_PROTECT;
		RUNPLAN;	  
		...
		//  lcd_setstatus(args, true);
		//else {
		  LCD_MESSAGEPGM(MSG_USERWAIT_OVERHEAT);
		  #if ENABLED(LCD_PROGRESS_BAR) && PROGRESS_MSG_EXPIRE > 0
			dontExpireStatus();
		  #endif
		//}
		...
		if (codenum > 0) {
		  codenum += previous_cmd_ms;  // wait until this time for a click
		  KEEPALIVE_STATE(PAUSED_FOR_OVERHEAT);
		  while (PENDING(millis(), codenum) && !lcd_clicked()) {idle();lifetime_stats_tick();}
		  KEEPALIVE_STATE(IN_HANDLER);
		  lcd_ignore_click(false);
		}
...
*
*/
#if ENABLED(FDM_Z_TRIAD)
	
	inline void gcode_M011(){
		
	}
	
	inline void gcode_M012(){
		// Wait for planner moves to finish!
		st_synchronize();
		if (code_seen('P'))				
			...
			{
				// case 0:
				// bed_level_point_a=false;
				// bed_level_point_b=false;
				// bed_level_point_c=false;
				// break;
				case 1:
				bed_level_point_a=true;
				bed_level_point_b=false;
				bed_level_point_c=false;
				//move xy to Z
				...
				case 2:	
				bed_level_point_a=false;
				bed_level_point_b=true;
				bed_level_point_c=false;
				//move xy to Z2
				destination[X_AXIS] = (X_MIN_POS + X_MAX_POS);
				destination[Y_AXIS] = (Y_MIN_POS + Y_MAX_POS);
				RUNPLAN;
				st_synchronize();
				enqueue_and_echo_commands_P(PSTR("G28 Z0"));
				break;
...
#if (ENABLED(GALVO_LASER)|| ENABLED(LASER_ENGRAVING_MODULE))
	inline void	gcode_M002(){
			LASER_POWER = code_seen('S') ? constrain(code_value_short(),0,255):255;
			laserPower = LASER_POWER;
	}
	
	inline void	gcode_M003(){ 
		Laser_Always_On=true;
	}
	
	inline void	gcode_M004(){
		Laser_Always_On=false;
	}	

	#if ENABLED(SLS_PRINTER)	
	
		inline void	gcode_M131(){
			if (code_seen('S')){
				sls_steps = code_value();	
			}			
			else {
				sls_steps = 3000;
			}
...
/* 			if (digitalRead(S_ENDSTOP_MIN_PIN)==0){
			//	steps = 1000;
				digitalWrite(R_DIR_PIN,HIGH);
			}
			if (digitalRead(S_ENDSTOP_MAX_PIN)==0){
				steps = 0;
				digitalWrite(R_DIR_PIN,LOW);
				
			} */
			for(int i=0;i<sls_steps;i++){
		...
		inline void	gcode_M130(){	
		
			WRITE(S_ENDSTOP_MIN_PIN,HIGH);
			WRITE(S_ENDSTOP_MAX_PIN,HIGH);
			
			digitalWrite(S_ENABLE_PIN,LOW);				
			

			if(feedrate_multiplier<=10) sls_delay=500;
			else sls_delay=100;
			delay(sls_delay);
			analogWrite(LASER_PIN,0); //NAMADD
			
			delay(1000);		
			if(!sls_dir)	
			{
				while (digitalRead(S_ENDSTOP_MIN_PIN)!=0){
			...
			{
				while (digitalRead(S_ENDSTOP_MAX_PIN)!=0){
		...
		inline void	gcode_M132(){
			...
			if (code_seen('S')){
				analogWrite(S_STEP_PIN,code_value());
			  }
			  else {
				analogWrite(S_STEP_PIN,254);
			  }	
		}
		
		inline void	gcode_M133(){	
  ...
  switch (command_code) {
    case 'G': switch (codenum) {
      ...
      case 28: // G28: Home all axes, one at a time
        gcode_G28();
		#if ENABLED(GALVO_LASER) 		//NAMADD Galvo phải nằm sau lệnh này 
			move_galvos(0,0); 			// GALVO HOME
		#endif		
        break;
    ...
    case 'M': switch (codenum) {
      ...
      #if ENABLED(DELTA) || ENABLED(Z_DUAL_ENDSTOPS) || ENABLED(Z_TRIAD_ENDSTOPS)
	...
	//NAMADD
	  #if ENABLED(LED_RING)
	  case 9:  //NAMADD LED_RING
		gcode_M009();
		break;
	  #endif
	  
	  case 10:
		gcode_M010(); //M010: Filament check
		break;	
	  case 5:
		gcode_M005(); //M005: Complete state
		break;	
	  case 6:
		gcode_M006(); //M006: Pause
		break;
	  #if ENABLED(MAXTEMP_PROTECT)
		  case 7:
			gcode_M007();
	  ...
	  #if (ENABLED(GALVO_LASER)||ENABLED(LASER_ENGRAVING_MODULE))  //NAMADD
		  case 002:  //Laser control //M002 laser 	
			gcode_M002();
			break;	
			
		  case 003:  //M003 Laser all turn on
			gcode_M003();
			break;	
